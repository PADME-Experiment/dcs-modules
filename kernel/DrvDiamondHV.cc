#include "DrvDiamondHV.h"   //in c file
...
using namespace std;
  ...
  DrvPadmeDiamond_except&
DrvPadmeDiamond_except::PadmeDiamondRetStatus(int ambhandler,int retstatus, const std::string& add)
{
 std::string error=(
  ...
  std::string str("PadmeDiamondWrapper returned error ("+std::to_string(retstatus)+") :  "+error);
  if(add.size()>0)str+="   "+add;

  switch (retstatus){
    case 1             : throw DrvPadmeDiamond_except::Except_SYSERR               (str) ;break;
    case 2             : throw DrvPadmeDiamond_except::Except_WRITEERR             (str) ;break;
    case 3             : throw DrvPadmeDiamond_except::Except_READERR              (str) ;break;
    case 4             : throw DrvPadmeDiamond_except::Except_TIMEERR              (str) ;break;
    case 5             : throw DrvPadmeDiamond_except::Except_DOWN                 (str) ;break;
    case 6             : throw DrvPadmeDiamond_except::Except_NOTPRES              (str) ;break;
  }
}

  int
DrvPadmeDiamond::ComInit()
{
...
//char *serv;
  ...
  if(fPort.size()!=0) {
    ...
    if (rc == MEMCACHED_SUCCESS)
      ...
      DrvPadmeDiamond_except::PadmeDiamondRetStatus(handle,6, "IP = "+fIPAddress+" ");
    }
  ...
  void
DrvPadmeDiamond::ComDeinit(int handle)
{
  // close(handle);
  // int ret = PadmeDiamond_DeinitSystem(handle);
  // if(ret != AMB_OK)
  // DrvPadmeDiamond_except::PadmeDiamondRetStatus(handle,ret);
}



  void
DrvPadmeDiamond::AssertInit()
{
  INFO("DrvPadmeDiamond::AssertInit()");
  //#warning CAEN ComInit
  fPadmeDiamondHandle=ComInit();
  AssertInitAllOwned();
  //ProcessUpdateListTemp();
}

void
DrvPadmeDiamond::Finalize()
{
  INFO("DrvPadmeDiamond::Finalize()");
  
  int bufsize;
  this->VDeviceBase::Finalize();
  //#warning CAEN ComDeinit
  ComDeinit(fPadmeDiamondHandle);
  JoinThread();
}

void
DrvPadmeDiamond::DebugDump()
{
 ...
 //char *serv;
  ...
  if (rc == MEMCACHED_SUCCESS) {
    ...
    cout <<  " PADME Amb memcache key " << key << " server says " ;
      ...
      // wait 3 secs for next read
      usleep(300000);

    } else {
    fprintf(stderr, "Couldn't retrieve key %s: %s\n", key,memcached_strerror(memc, rc));
      DrvPadmeDiamond_except::PadmeDiamondRetStatus(handle,3, "IP = "+fIPAddress+" ");
